...
// Deletes a mapping in the vregion from the virtual address from_va of size sz with the appropriate
// permissions
int
...
// Copy-on-write version of copy_vpi_page. Recursively copies the vpi_page
// struct from src to dst without allocating new physical frames for pages.
//
// return 0 on success, -1 if failed
static int
cow_copy_vpi_page(struct vpi_page **dst, struct vpi_page *src)
{
  ...
  for (i = 0; i < VPIPPAGE; i++) {
    ...
    if (srcvpi->used) {
      ...
      // Increment the reference count of the physical frame by 1
      struct core_map_entry *r = (struct core_map_entry *)
                                      pa2page(srcvpi->ppn << PT_SHIFT);
      acquire_core_map_lock();
      r->refer_count++;
      release_core_map_lock();

      // If the src page is already read-only, do not set the page to be
      // COW page
      if (srcvpi->writable == VPI_READONLY && !srcvpi->is_cow) {
...
// Copy-on-write version of vspacecopy. Instead of allocating a new physical
// frame for each page, let it point to the physical frame of the corresponding
// page of src.
int
