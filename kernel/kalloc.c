...
// Acquire the lock for the core map
void acquire_core_map_lock() {
  if (kmem.use_lock)
    acquire(&kmem.lock);
}

// Release the lock for the core map
void release_core_map_lock() {
  if (kmem.use_lock)
    release(&kmem.lock);
}
...
void kfree(char *v) {
  ...
  // Decrement the reference count by 1 if there are still pages referencing it
  if (r->refer_count >= 1)
    r->refer_count--;

  // Actually free the physical frame when there are no pages referencing it
  if (r->refer_count == 0) {
    pages_in_use--;
    free_pages++;
